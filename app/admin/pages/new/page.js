...
import {
  Save, Send, ChevronDown, ChevronUp, Image as ImageIcon,
  X, Calendar, Lock, Globe, Layers,
} from 'lucide-react'
  ...
  async function handleSave(saveStatus) {
    ...
    if (postIdRef.current) {
      const res = await fetch(`/api/posts/${postIdRef.current}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(postData),
      })
      ...
      savedPost = (await res.json()).data
      if (savedPost?.id) postIdRef.current = savedPost.id
    }

    setSaving(false)
    if (savedPost) router.push(`/admin/pages/${savedPost.id}/edit`)
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>Add New Page</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '1.25rem', alignItems: 'start' }}>
        <div>
          ...
          <Editor content={contentJson} onChange={handleEditorChange} placeholder="Start writing your page…" />
        </div>
