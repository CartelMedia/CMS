...
export async function generateMetadata({ params }) {
  ...
  const { data } = await supabase.from('tags').select('name').eq('slug', slug).maybeSingle()
  if (!data) return { title: 'Tag Not Found' }
  return { title: `#${data.name} — Tag`, description: `Posts tagged with ${data.name}` }
}
...
export default async function TagPage({ params, searchParams }) {
  ...
  const posts = (postTags || []).map(pt => pt.post).filter(p => p?.published_at)
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '8px' }}>
          ...
          <div>
            <p style={{ color: '#8b5cf6', fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>Tag</p>
            <h1 style={{ color: '#f1f5f9', fontSize: '28px', fontWeight: 800, margin: 0, letterSpacing: '-0.01em' }}>#{tag.name}</h1>
          </div>
        </div>
        <p style={{ color: '#475569', fontSize: '13px', margin: '8px 0 0 62px' }}>
          {count || 0} post{count !== 1 ? 's' : ''} tagged with &ldquo;{tag.name}&rdquo;
        </p>
      </div>
      ...
      {posts.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontSize: '14px', margin: 0 }}>No posts with this tag yet.</p>
        </div>
      ) : (
              ...
              <Link key={post.id} href={`/${post.slug}`} style={{ textDecoration: 'none' }}>
                ...
                >
                  {post.featured_image ? (
                    ...
                    <div style={{ height: '140px', background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(99,102,241,0.06))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <BookOpen size={36} color="rgba(139,92,246,0.3)" />
                    </div>
                  )}
                  <div style={{ padding: '18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', color: '#475569', fontSize: '12px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '12px', marginTop: 'auto' }}>
                      ...
                      <span style={{ marginLeft: 'auto', color: '#8b5cf6', fontSize: '12px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px' }}>Read <ArrowRight size={11} /></span>
                    </div>
                  </div>
                </article>
              </Link>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '40px' }}>
          {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
            <Link key={p} href={`/tag/${slug}?page=${p}`} style={{
              width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center',
              borderRadius: '8px', border: '1px solid',
              borderColor: p === page ? 'rgba(139,92,246,0.5)' : 'rgba(255,255,255,0.1)',
              background: p === page ? 'rgba(139,92,246,0.15)' : 'none',
