...
async function getData() {
  ...
  const [settingsRes, postsRes, categoriesRes, featuredRes] = await Promise.all([
    supabase.from('options').select('option_key, option_value').in('option_key', ['site_title', 'tagline', 'posts_per_page']),
    supabase.from('posts')
    ...
    return (
      <Link href={`/${post.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
        <div style={{
          ...
          onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 40px rgba(0,0,0,0.4)' }}
        >
          <span style={{
          ...
          <h2 style={{ color: '#f1f5f9', fontSize: '28px', fontWeight: 800, lineHeight: 1.25, margin: '0 0 12px', letterSpacing: '-0.01em' }}>
            {post.title}
          </h2>
          <p style={{ color: '#94a3b8', fontSize: '15px', lineHeight: 1.6, margin: '0 0 20px' }}>
            {excerpt}
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', color: '#64748b', fontSize: '13px' }}>
            ...
            <span style={{
              ...
              marginLeft: 'auto', color: '#a5b4fc', fontWeight: 600,
            }}>
    ...
    <Link href={`/${post.slug}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
      ...
      >
        ...
        <div style={{ padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <h3 style={{ color: '#f1f5f9', fontSize: '17px', fontWeight: 700, lineHeight: 1.35, margin: '0 0 10px', letterSpacing: '-0.01em' }}>
            {post.title}
          </h3>

          <p style={{ color: '#64748b', fontSize: '13px', lineHeight: 1.65, margin: '0 0 16px', flex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#475569', fontSize: '12px', paddingTop: '14px', borderTop: '1px solid rgba(255,255,255,0.05)', marginTop: 'auto' }}>
            ...
            <span style={{ marginLeft: 'auto', color: '#6366f1', fontSize: '12px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px' }}>
...
export async function generateMetadata() {
  ...
  const settings = {}
  data?.forEach(row => { settings[row.option_key] = row.option_value })
  return {
    title: settings.site_title || 'ModernCMS',
    description: settings.tagline || 'A modern content management system',
  }
}
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '32px', alignItems: 'center' }}>
          ...
          {categories.map(cat => (
            <Link key={cat.slug} href={`/category/${cat.slug}`} style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(99,102,241,0.12)'; e.currentTarget.style.color = '#a5b4fc'; e.currentTarget.style.borderColor = 'rgba(99,102,241,0.3)' }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        ...
        <Link href="/search" style={{ color: '#6366f1', textDecoration: 'none', fontSize: '13px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '4px' }}>
